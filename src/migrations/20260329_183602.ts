...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`posts_collaborators\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`posts_collaborators_order_idx\` ON \`posts_collaborators\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`posts_collaborators_parent_id_idx\` ON \`posts_collaborators\` (\`_parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`posts_collaborators\`;`)
